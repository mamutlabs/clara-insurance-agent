...
// Firebase Imports (solo para autenticación del cliente)
...
// --- Firebase Configuration (solo para el cliente) ---
...
function App() {
    ...
    const callApi = async (action, payload) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(err.details || 'Error en la llamada a la API');
            }
            return await response.json();
        } catch (error) {
            console.error(`API Error on action ${action}:`, error);
            setMessages(prev => [...prev, { role: 'system', isSuccess: false, content: `Error en el backend: ${error.message}` }]);
            return null;
        } finally {
            setIsLoading(false);
        }
    ...
    const handleFileChange = async (e) => {
        ...
        const result = await callApi('processDocument', { fileName: file.name, fileContent, userEmail: user.email });
        if (result) {
            setMessages(prev => [...prev, { role: 'system', isSuccess: true, content: `¡Éxito! ${result.chunkCount} fragmentos de "${file.name}" han sido añadidos a la memoria de Clara.` }]);
        }
    };
